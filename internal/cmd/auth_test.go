...
func TestAuthLogin_NoBrowserFlagAvailable(t *testing.T) {
	app := NewApp()
	authCmd := newAuthCmd(app)

	if flag := authCmd.PersistentFlags().Lookup("no-browser"); flag == nil {
		t.Fatalf("expected auth command to define --no-browser flag")
	}
	...
	if flag := loginCmd.Flag("no-browser"); flag == nil {
		t.Fatalf("expected auth login command to expose --no-browser flag")
	}
}
